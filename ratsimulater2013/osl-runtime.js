...
(() => {
  ...
  function tick() {
    if (programScope) {
      ...
      programScope.mouse_ondown = state.mouseOnDown;
    ...
    prevTree = ctx.buffer;
    justKeys.clear();
    if (state.mouseOnDown) state.mouseOnDown = false;
  }
  ...
  function soundInfo(a, b) {
    ...
    if (val === "loaded") {
      return snd.readyState == 4;
    }
    if (val === "playing") {
      return !snd.paused && !snd.ended && snd.currentTime > 0;
    }
    if (val === "volume") {
      return snd.volume;
    }
